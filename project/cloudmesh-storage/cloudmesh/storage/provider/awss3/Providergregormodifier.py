...
class Provider(StorageABC):
    ...
    def put(self, source=None, destination=None, recursive=False):
        ...
        self.storage_dict['root'] = "???" # read this from the yaml default wehn looking up the storage provide by name which is src_service
        self.storage_dict['source'] = source #src do the same thing for the destination
        self.storage_dict['destination'] = destination #dest
        self.storage_dict['recursive'] = recursive
        pprint(self.storage_dict)

        queue = # create the queue as gregor showded in the example and push all files in the recursive tree as tuple
        ...
        if recursive
                quees = ... list source and generate the path for destination queu.acton == "copy" source, desination".
                self.run (queue)

        loop through the queue and

        if action == "put":
            than do what we do in this routine, but as we do not need
            recusrive here as its in the queu generation its much simple, it just does one put at a time
